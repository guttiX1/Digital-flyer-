...
const HTML = `file://${path.resolve(__dirname, 'events/pesos-pesados-jul12/video-ad.html')}`;
const FRAMES_DIR = '/tmp/vid-frames-pesos';
const OUT_VIDEO = path.resolve(__dirname, 'events/pesos-pesados-jul12/pesos-pesados-video-ad.mp4');
...
(async () => {
  ...
  const intermediate = '/tmp/intermediate-pesos.mp4';
  await ffmpegRun([
    '-y',
    '-framerate', sourceFPS,
  ...
  const interpolated = '/tmp/interpolated-pesos.mp4';
  await ffmpegRun([
    '-y',
    '-i', intermediate,
