...
const HTML = `file://${path.resolve(__dirname, 'events/carril-potosino-ago9/video-ad.html')}`;
const FRAMES_DIR = '/tmp/vid-frames-poto';
const OUT_VIDEO = path.resolve(__dirname, 'events/carril-potosino-ago9/potosino-video-ad.mp4');
...
(async () => {
  ...
  const intermediate = '/tmp/intermediate-poto.mp4';
  await ffmpegRun([
    '-y',
    '-framerate', sourceFPS,
  ...
  const interpolated = '/tmp/interpolated-poto.mp4';
  await ffmpegRun([
    '-y',
    '-i', intermediate,
